      ...
      if (_appointment.value.status!.order == Get.find<GlobalService>().global.value.onTheWay) {
        ...
        return Container(
          ...
          child: Row(mainAxisAlignment: MainAxisAlignment.end, children: [
            ...
            if (_appointment.value.status!.order >= Get.find<GlobalService>().global.value.done! && _appointment.value.status!.order !=60 &&_appointment.value.payment != null)
              Expanded(
                child: BlockButtonWidget(
                    text: Stack(
                      alignment: AlignmentDirectional.centerEnd,
            ...
            SizedBox(width: 10),
                                                                                                                                  // كان موجود الشرط داخل الدالة
            if (!_appointment.value.cancel && _appointment.value.status!.order <= Get.find<GlobalService>().global.value.accepted! )//&& !(_appointment.value.payment?.hasData ?? false)
              MaterialButton(
                onPressed: () {
                ...
                color: Colors.red,
                child: Text("Cancel appointment".tr, style: TextStyle(color: Colors.white,fontSize: 14,fontWeight: FontWeight.bold),),
