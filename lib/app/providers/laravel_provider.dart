...
class LaravelApiClient extends GetxService with ApiClient {
  ...
  Future<User> checkOtpAndLogin(User user,String smsCode) async {
    Uri _uri = getApiBaseUri("check_otp_and_login");
    Get.log(_uri.toString());
    ...
    if (response.data['success'] == true) {
      ...
      throw new Exception(response.data['message'].toString().tr);
    }
  }

  Future<User> updateUserWithOutOtp(User user) async {
    if (!authService.isAuth) {
    ...
    Uri _uri = getApiBaseUri("userOtp/{user}/${user.id}").replace(queryParameters: _queryParameters);
    Get.log(_uri.toString());
    var response = await httpClient.postUri(
      _uri,
      data: json.encode(user.toJson()),
  ...
  Future<String> sendOtpUserProfile(User user) async {
    Uri _uri = getApiBaseUri("send_otp_user_profile");
    Get.log(_uri.toString());
    var response = await httpClient.postUri(
      _uri,
        data: json.encode(user.toJson()),
...
/* Future<User> login(User user) async {//الدخول قبل تعديلي
  ...
  Future<String> checkUser(User user) async {
    Uri _uri = getApiBaseUri("check_user");
    Get.log(_uri.toString());
    var response = await httpClient.postUri(
      _uri,
      data: json.encode(user.toJson()),
  ...
  Future<User> loginWithOutOtp(User user) async {
    Uri _uri = getApiBaseUri("login_with_out_otp");
    Get.log(_uri.toString());
    Map<String, String> body = {
      'user':json.encode(user.toJson()),
    };
  ...
  Future<String> registerByPhone(User user) async {
    Uri _uri = getApiBaseUri("register_by_phone");
    Get.log(_uri.toString());
    var response = await httpClient.postUri(
      _uri,
      data: json.encode(user.toJson()),
  ...
  Future<String> resendOTPCode(User user,String smsCode) async {
    Uri _uri = getApiBaseUri("resend_otp_code");
    Get.log(_uri.toString());
    Map<String, String> body = {
      'otp_code': smsCode,
      'user':json.encode(user.toJson()),
    ...
    if (response.data['success'] == true) {
        return response.data['message'].toString().tr;
    } else {
  ...
  /*Future<User> resendOTPCode(User user,String smsCode) async {
    Uri _uri = getApiBaseUri("resend_otp_code");
    Get.log(_uri.toString());
    Map<String, String> body = {
      'otp_code': smsCode,
  ...
  Future<User> updateUser(User user,[String smsCode="0"]) async {
    if (!authService.isAuth) {
      throw new Exception("You don't have the permission to access to this area!".tr + "[ updateUser() ]");
    }
    var _queryParameters = {
  ...
  Future<List<ProcedureGuide>> getAllProcedureGuides() async {

    Uri _uri = getApiBaseUri("procedure_guides");
    Get.log(_uri.toString());
    var response = await httpClient.getUri(_uri, options: optionsCache);
    if (response.data['success'] == true) {
      return response.data['data'].map<ProcedureGuide>((obj) => ProcedureGuide.fromJson(obj)).toList();
    } else {
      throw new Exception(response.data['message']);
    }
  }
  ...
  Future<Appointment> addAppointment(Appointment appointment) async {
    ...
    if (response.data['success'] == true) {
      print("addAppointment :${appointment.toJson()}");
      return Appointment.fromJson(response.data['data']);
    } else {
      print("addAppointment :${appointment.toJson()}");
      throw new Exception(response.data['message'].toString().tr);
  ...
  Uri getUrWayUrl(Appointment _appointment) {
    if (!authService.isAuth) {
      throw new Exception("You don't have the permission to access to this area!".tr + "[ getUrWayUrl() ]");
    }
    var _queryParameters = {
      'api_token': authService.apiToken,
      'doctor_id': _appointment.doctor?.id,
    };
    Uri _uri = getBaseUri("payments/mada/checkout").replace(queryParameters: _queryParameters);
    return _uri;
  }
  ...
  Future<List> getAvailabilityHours(String doctorId, DateTime date) async {
    ...
    var response = await httpClient.getUri(_uri, options: optionsNetwork);
    if (response.data['success'] == true) {
      print("Time ${response.data['data'][1][2]}");
      return response.data['data'];
    } else {
      throw new Exception(response.data['message']);
    }
